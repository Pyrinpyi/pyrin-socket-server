...
from .messages_pb2 import PyipadMessage


MAX_MESSAGE_LENGTH = 1024 * 1024 * 1024  # 1GB


class PyipadCommunicationError(Exception): pass


# pipenv run python -m grpc_tools.protoc -I./protos --python_out=. --grpc_python_out=. ./protos/rpc.proto ./protos/messages.proto ./protos/p2p.proto

class PyipadThread(object):
    def __init__(self, pyrin_host, pyrin_port, async_thread=True):

        self.pyrin_host = pyrin_host
        self.pyrin_port = pyrin_port

        if async_thread:
            self.channel = grpc.aio.insecure_channel(f'{pyrin_host}:{pyrin_port}',
            ...
            self.channel = grpc.insecure_channel(f'{pyrin_host}:{pyrin_port}',
                                                 compression=grpc.Compression.Gzip,
                                                 options=[
                                                     ('grpc.max_send_message_length', MAX_MESSAGE_LENGTH),
                                                     ('grpc.max_receive_message_length', MAX_MESSAGE_LENGTH),
    ...
    async def request(self, command, params=None, wait_for_response=True, timeout=120):
        if wait_for_response:
            try:
                ...
                raise PyipadCommunicationError(str(e))
